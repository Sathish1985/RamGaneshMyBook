	...
	function createFeed(){
		...
		var postText = document.getElementById('postText');
		 var pattern = new RegExp('^(https?:\\/\\/)?'+ 
		  ...
		  '(\\#[-a-z\\d_]*)?$','i');
		if(postText.value !=''){
			if(pattern.test(postText.value)){
			var patterned = /^((http|https|ftp):\/\/)/;

				if(!patterned.test(postText.value)) {
				  var link="<a href='http://"+postText.value+"' target='_blank'>"+postText.value+"</a>";
				}
				else {
				var link=postText.value;
				}
	...
	function checkValidation(){
		...
		if(nameFlag== true && ageFlag == true && phoneFlag == true && emailFlag == true){
			document.getElementById('saveProfile').removeAttribute("disabled");
			console.log('enable');
		}else{
